...
namespace LiteBulb.Algorithms.Application.Services.NthLargestElement
{
    ...
    public class SortedListOptimizedStrategy<T> : IStrategy<T> where T : IHasValue, new()
    {
        public SortedListOptimizedStrategy()
        {
        }
        ...
        public T Find(IReadOnlyCollection<T> elements, int n)
        {
            ...
            {
                // if list has enough elements
                if (sortedList.Count >= n)
                {
                    var min = sortedList.GetValueAtIndex(0);
